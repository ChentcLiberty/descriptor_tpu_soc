...
module tb_tpu_stage2_cnn_frontend_smoke;
    ...
    localparam [31:0] CONV3_W_BASE    = 32'h6007_5400;
    localparam [31:0] CONV3_B_BASE    = 32'h6008_1400;
    localparam [31:0] CONV4_W_BASE    = 32'h6008_1500;
    localparam [31:0] CONV4_B_BASE    = 32'h600B_1500;
    ...
    localparam integer CONV3_IN_CH = 2;
    localparam integer CONV3_IN_LEN = 2;
    localparam integer CONV3_OUT_CH = 2;
    localparam integer CONV3_KERNEL = 3;
    localparam integer CONV3_PAD = 1;
    localparam integer CONV3_WEIGHT_WORDS = 6;
    localparam integer CONV3_BIAS_WORDS = 1;

    localparam integer CONV4_IN_CH = 2;
    localparam integer CONV4_IN_LEN = 1;
    localparam integer CONV4_OUT_CH = 2;
    localparam integer CONV4_KERNEL = 3;
    localparam integer CONV4_PAD = 1;
    localparam integer CONV4_WEIGHT_WORDS = 6;
    localparam integer CONV4_BIAS_WORDS = 1;
    ...
    localparam integer FINAL_OUTPUT_WORDS = 1;
    localparam integer TOTAL_PARAM_WORDS =
        CONV1_WEIGHT_WORDS + CONV1_BIAS_WORDS +
        CONV2_WEIGHT_WORDS + CONV2_BIAS_WORDS +
        CONV3_WEIGHT_WORDS + CONV3_BIAS_WORDS +
        CONV4_WEIGHT_WORDS + CONV4_BIAS_WORDS +
    ...
    reg [31:0] conv3_weight_words[0:CONV3_WEIGHT_WORDS-1];
    reg [31:0] conv3_bias_words  [0:CONV3_BIAS_WORDS-1];
    reg [31:0] conv4_weight_words[0:CONV4_WEIGHT_WORDS-1];
    reg [31:0] conv4_bias_words  [0:CONV4_BIAS_WORDS-1];
    ...
    tpu_stage2_cnn_frontend_wrapper #(
        ...
        .CONV3_W_BASE(CONV3_W_BASE),
        .CONV3_B_BASE(CONV3_B_BASE),
        .CONV4_W_BASE(CONV4_W_BASE),
        .CONV4_B_BASE(CONV4_B_BASE),
        ...
        .CONV3_WEIGHT_WORDS(CONV3_WEIGHT_WORDS),
        .CONV3_BIAS_WORDS(CONV3_BIAS_WORDS),
        .CONV4_WEIGHT_WORDS(CONV4_WEIGHT_WORDS),
        .CONV4_BIAS_WORDS(CONV4_BIAS_WORDS),
    ...
    function [31:0] bus_read_word;
        ...
        begin
            ...
            end else if ((addr >= CONV3_W_BASE) && (addr < (CONV3_W_BASE + CONV3_WEIGHT_WORDS * 4))) begin
                idx = (addr - CONV3_W_BASE) >> 2;
                bus_read_word = conv3_weight_words[idx];
            end else if ((addr >= CONV3_B_BASE) && (addr < (CONV3_B_BASE + CONV3_BIAS_WORDS * 4))) begin
                idx = (addr - CONV3_B_BASE) >> 2;
                bus_read_word = conv3_bias_words[idx];
            end else if ((addr >= CONV4_W_BASE) && (addr < (CONV4_W_BASE + CONV4_WEIGHT_WORDS * 4))) begin
                idx = (addr - CONV4_W_BASE) >> 2;
                bus_read_word = conv4_weight_words[idx];
            end else if ((addr >= CONV4_B_BASE) && (addr < (CONV4_B_BASE + CONV4_BIAS_WORDS * 4))) begin
                idx = (addr - CONV4_B_BASE) >> 2;
                bus_read_word = conv4_bias_words[idx];
    ...
    task clear_storage;
        begin
            ...
            for (i = 0; i < CONV3_WEIGHT_WORDS; i = i + 1) begin
                conv3_weight_words[i] = 32'd0;
            end
            for (i = 0; i < CONV3_BIAS_WORDS; i = i + 1) begin
                conv3_bias_words[i] = 32'd0;
            end
            for (i = 0; i < CONV4_WEIGHT_WORDS; i = i + 1) begin
                conv4_weight_words[i] = 32'd0;
            end
            for (i = 0; i < CONV4_BIAS_WORDS; i = i + 1) begin
                conv4_bias_words[i] = 32'd0;
            end
    ...
    initial begin
        ...
        conv3_weight_words[0] = pack_q16(16'd0, 16'd256);
        conv3_weight_words[1] = pack_q16(16'd0, 16'd0);
        conv3_weight_words[2] = pack_q16(16'd0, 16'd0);
        conv3_weight_words[3] = pack_q16(16'd0, 16'd0);
        conv3_weight_words[4] = pack_q16(16'd0, 16'd0);
        conv3_weight_words[5] = pack_q16(16'd256, 16'd0);
        conv3_bias_words[0]   = pack_q16(16'd0, 16'd0);

        conv4_weight_words[0] = pack_q16(16'd0, 16'd256);
        conv4_weight_words[1] = pack_q16(16'd0, 16'd0);
        conv4_weight_words[2] = pack_q16(16'd0, 16'd0);
        conv4_weight_words[3] = pack_q16(16'd0, 16'd0);
        conv4_weight_words[4] = pack_q16(16'd0, 16'd0);
        conv4_weight_words[5] = pack_q16(16'd256, 16'd0);
        conv4_bias_words[0]   = pack_q16(16'd0, 16'd0);
        ...
        if (output_words_mem[0] != 32'h0980_3300) begin
