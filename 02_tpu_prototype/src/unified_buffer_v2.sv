// Unified Buffer V2 - Fixed version
// Changes from fixed version:
// 1. Fixed rst timing issue: added hold cycle before resetting outputs
// 2. Fixed blocking/non-blocking assignment mixing: all ptr calculations moved to combinational logic
...
module unified_buffer #(
    ...
    // Debug wires: Icarus VCD cannot dump unpacked-array ports, mirror them here
    ...
    logic [15:0] wr_ptr;
    logic [15:0] wr_ptr_next;  // Combinational next value

    // Internal logic for reading inputs from UB to left side of systolic array
    logic [15:0] rd_input_ptr;
    logic [15:0] rd_input_ptr_next;  // Combinational next value
    ...
    logic signed [15:0] rd_weight_ptr_next;  // Combinational next value
    ...
    logic [15:0] rd_Y_ptr;
    logic [15:0] rd_Y_ptr_next;  // Combinational next value
    ...
    logic [15:0] rd_H_ptr;
    logic [15:0] rd_H_ptr_next;  // Combinational next value
    ...
    logic [15:0] rd_grad_weight_ptr_next;  // Combinational next value
    ...
    logic [15:0] grad_descent_ptr_next;  // Combinational next value
    ...
    // These replace the blocking assignment ptr increments in the original code
    logic [15:0] wr_lane_addr [SYSTOLIC_ARRAY_WIDTH];
    logic [15:0] rd_input_lane_addr [SYSTOLIC_ARRAY_WIDTH];
    logic [15:0] rd_input_lane_addr_t [SYSTOLIC_ARRAY_WIDTH];  // transposed
    logic signed [15:0] rd_weight_lane_addr [SYSTOLIC_ARRAY_WIDTH];
    logic signed [15:0] rd_weight_lane_addr_t [SYSTOLIC_ARRAY_WIDTH];  // transposed
    ...
    // All pointer updates are calculated here to avoid blocking/non-blocking mixing

    // wr_ptr_next and per-lane write addresses
    always_comb begin
        wr_ptr_next = wr_ptr;
        // For loop decrements: lane1(j=1) first, lane0(j=0) second
    ...
    always_comb begin
        ...
        // Untransposed: for loop decrements
        for (int j = SYSTOLIC_ARRAY_WIDTH-1; j >= 0; j--) begin
    ...
    // rd_input per-lane read addresses (transposed)
    always_comb begin
        // Transposed: for loop increments, but shares same ptr_next
        // We need a separate calculation for transposed addresses
        begin
            logic [15:0] t_ptr;
            t_ptr = rd_input_ptr;
            for (int j = 0; j < SYSTOLIC_ARRAY_WIDTH; j++) begin
    ...
    always_comb begin
        ...
        if (rd_weight_time_counter < rd_weight_row_size + rd_weight_col_size) begin
            if(rd_weight_transpose) begin
                // Transposed: for loop increments
                for (int j = 0; j < SYSTOLIC_ARRAY_WIDTH; j++) begin
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // Use pre-calculated per-lane addresses
            for (int j = SYSTOLIC_ARRAY_WIDTH-1; j >= 0; j--) begin
