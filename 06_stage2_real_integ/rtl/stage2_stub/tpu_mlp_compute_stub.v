...
module tpu_mlp_compute_stub #(
    ...
    reg signed [47:0] linear_acc0;
    reg signed [47:0] linear_acc1;
    ...
    reg signed [31:0] linear_shifted0;
    reg signed [31:0] linear_shifted1;
    ...
    function signed [31:0] q8_8_mul_full;
        input signed [15:0] a;
        input signed [15:0] b;
        reg signed [31:0] product;
        begin
            product = a * b;
            q8_8_mul_full = product;
        end
    endfunction
    ...
    always @(*) begin
            ...
            if((linear_input_idx < input_words) &&
               ((linear_param_base + (linear_input_idx << 1) + 32'd1) < PARAM_MEM_WORDS)) begin
                ...
                linear_acc0        = linear_acc0 +
                                     q8_8_mul_full(linear_x0, linear_w00) +
                                     q8_8_mul_full(linear_x1, linear_w01);
                linear_acc1        = linear_acc1 +
                                     q8_8_mul_full(linear_x0, linear_w10) +
                                     q8_8_mul_full(linear_x1, linear_w11);
            end
        end
        ...
        linear_acc0 = linear_acc0 + ($signed({{32{linear_b0[15]}}, linear_b0}) <<< 8);
        linear_acc1 = linear_acc1 + ($signed({{32{linear_b1[15]}}, linear_b1}) <<< 8);
        linear_shifted0 = q8_8_round_shift_32(linear_acc0);
        linear_shifted1 = q8_8_round_shift_32(linear_acc1);

        if(flags[TPU_DESC_F_RELU_BIT] && (linear_shifted0 < 32'sd0)) begin
            linear_y0 = 16'sd0;
        end else begin
            linear_y0 = q8_8_saturate(linear_shifted0);
        end

        if(flags[TPU_DESC_F_RELU_BIT] && (linear_shifted1 < 32'sd0)) begin
            linear_y1 = 16'sd0;
        end else begin
            linear_y1 = q8_8_saturate(linear_shifted1);
        end
