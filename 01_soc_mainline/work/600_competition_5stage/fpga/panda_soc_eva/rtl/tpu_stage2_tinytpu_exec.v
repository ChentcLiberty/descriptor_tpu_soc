...
module tpu_stage2_tinytpu_exec #(
    ...
    localparam integer SYS_PSUM_WIDTH               = 48;
    localparam signed [SYS_PSUM_WIDTH-1:0] Q8_8_ROUND_BIAS = {{(SYS_PSUM_WIDTH-8){1'b0}}, 8'd128};
    localparam signed [SYS_PSUM_WIDTH-1:0] Q8_8_MAX_INT32  = {{(SYS_PSUM_WIDTH-32){1'b0}}, 32'h7fff_ffff};
    localparam signed [SYS_PSUM_WIDTH-1:0] Q8_8_MIN_INT32  = {{(SYS_PSUM_WIDTH-32){1'b1}}, 32'h8000_0000};
    ...
    reg signed [SYS_PSUM_WIDTH-1:0] accum0_reg;
    reg signed [SYS_PSUM_WIDTH-1:0] accum1_reg;
    ...
    wire signed [SYS_PSUM_WIDTH-1:0] sys_psum_out_21;
    wire signed [SYS_PSUM_WIDTH-1:0] sys_psum_out_22;
    ...
    reg  signed [SYS_PSUM_WIDTH-1:0] final_accum0_comb;
    reg  signed [SYS_PSUM_WIDTH-1:0] final_accum1_comb;
    reg  signed [31:0] final_round0_comb;
    reg  signed [31:0] final_round1_comb;
    ...
    function signed [31:0] q8_8_round_shift_32;
        input signed [SYS_PSUM_WIDTH-1:0] value;
        reg signed [SYS_PSUM_WIDTH-1:0] rounded;
        begin
            if(value >= 0) begin
            ...
            if(rounded > Q8_8_MAX_INT32) begin
                q8_8_round_shift_32 = 32'sh7fff_ffff;
            end else if(rounded < Q8_8_MIN_INT32) begin
    ...
    always @(*) begin
        ...
        final_accum0_comb = accum0_reg + ($signed({{(SYS_PSUM_WIDTH-16){tile_bias_word_comb[15]}}, tile_bias_word_comb[15:0]}) <<< 8);
        final_accum1_comb = accum1_reg + ($signed({{(SYS_PSUM_WIDTH-16){tile_bias_word_comb[31]}}, tile_bias_word_comb[31:16]}) <<< 8);
        final_round0_comb = q8_8_round_shift_32(final_accum0_comb);
        final_round1_comb = q8_8_round_shift_32(final_accum1_comb);

        if(flags[TPU_DESC_F_RELU_BIT] && (final_round0_comb < 32'sd0)) begin
            final_word0_comb = 16'sd0;
        end else begin
            final_word0_comb = sat_q8_8_16(final_round0_comb);
        end

        if(flags[TPU_DESC_F_RELU_BIT] && (final_round1_comb < 32'sd0)) begin
            final_word1_comb = 16'sd0;
        end else begin
            final_word1_comb = sat_q8_8_16(final_round1_comb);
        end
    ...
    tpu #(
        ...
        .sys_psum_out_21(sys_psum_out_21),
        .sys_psum_out_22(sys_psum_out_22),
    ...
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            ...
            case(exec_state_reg)
                ...
                EX_WAIT_VALID: begin
                    if(sys_valid_out_21 && !tile_seen_out0_reg) begin
                        accum0_reg <= accum0_reg + sys_psum_out_21;
                        tile_seen_out0_reg <= 1'b1;
                    end

                    if(sys_valid_out_22 && !tile_seen_out1_reg) begin
                        accum1_reg <= accum1_reg + sys_psum_out_22;
                        tile_seen_out1_reg <= 1'b1;
                    end

                    if((tile_seen_out0_reg || sys_valid_out_21) &&
                       (tile_seen_out1_reg || sys_valid_out_22)) begin
