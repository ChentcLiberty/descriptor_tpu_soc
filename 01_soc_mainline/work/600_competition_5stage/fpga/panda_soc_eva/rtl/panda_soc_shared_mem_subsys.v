...
`ifndef SYNTHESIS
    initial begin
        if(SHARED_SRAM_INIT_FILE != "no_init") begin
            #(SHARED_SRAM_INIT_DELAY);
            $display("[%0t] shared SRAM init: loading %0s", $time, SHARED_SRAM_INIT_FILE);
            $readmemh(SHARED_SRAM_INIT_FILE, axi_ram_u.mem);
        end
    end
`endif
