...
module unified_buffer #(
    ...
    output logic        ub_rd_Y_valid_out_0,
    output logic        ub_rd_Y_valid_out_1,
    ...
    output logic        ub_rd_H_valid_out_0,
    output logic        ub_rd_H_valid_out_1,
...
`endif
    ...
    logic ub_rd_Y_valid_out [SYSTOLIC_ARRAY_WIDTH];
    logic ub_rd_H_valid_out [SYSTOLIC_ARRAY_WIDTH];
    ...
    assign ub_rd_Y_valid_out_0 = ub_rd_Y_valid_out[0];
    assign ub_rd_Y_valid_out_1 = ub_rd_Y_valid_out[1];

    assign ub_rd_H_data_out_0 = ub_rd_H_data_out[0];
    assign ub_rd_H_data_out_1 = ub_rd_H_data_out[1];
    assign ub_rd_H_valid_out_0 = ub_rd_H_valid_out[0];
    assign ub_rd_H_valid_out_1 = ub_rd_H_valid_out[1];
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            for (int j = 0; j < SYSTOLIC_ARRAY_WIDTH; j++) begin
                ...
                ub_rd_Y_valid_out[j] <= '0;
                ub_rd_H_data_out[j] <= '0;
                ub_rd_H_valid_out[j] <= '0;
                value_old_in[j] <= '0;
            end
            ...
            if (rd_bias_time_counter + 1 < rd_bias_row_size + rd_bias_col_size) begin
                for (int j = 0; j < SYSTOLIC_ARRAY_WIDTH; j++) begin
                    if(rd_bias_time_counter >= j && rd_bias_time_counter < rd_bias_row_size + j && j < rd_bias_col_size) begin
                        ...
                        // Bias scalars are per-column constants for the current tile.
                        // Keep the last fetched value live until the next reset/read so
                        // the VPU still sees it when the systolic outputs arrive.
                        ub_rd_bias_data_out[j] <= ub_rd_bias_data_out[j];
                    end
                end
                ...
                // Clear the read counters but keep the last bias scalars live until a
                // new tile reset or bias read overwrites them.
                rd_bias_ptr <= 0;
                rd_bias_row_size <= 0;
                rd_bias_col_size <= 0;
                rd_bias_time_counter <= '0;
            ...
            if (rd_Y_time_counter + 1 < rd_Y_row_size + rd_Y_col_size) begin
                for (int j = SYSTOLIC_ARRAY_WIDTH-1; j >= 0; j--) begin
                    if(rd_Y_time_counter >= j && rd_Y_time_counter < rd_Y_row_size + j && j < rd_Y_col_size) begin
                        ...
                        ub_rd_Y_valid_out[j] <= 1'b1;
                    end else begin
                        ub_rd_Y_data_out[j] <= ub_rd_Y_data_out[j];
                        ub_rd_Y_valid_out[j] <= 1'b0;
                    end
                end
                ...
                // Clear the read counters but keep the last Y scalars live until a
                // new Y read or reset overwrites them. This is needed for the
                // transition/loss pathway, where the VPU consumes Y after the read
                // command itself has already retired.
                rd_Y_ptr <= 0;
                rd_Y_row_size <= 0;
                rd_Y_col_size <= 0;
                rd_Y_time_counter <= '0;
            ...
            if (rd_H_time_counter + 1 < rd_H_row_size + rd_H_col_size) begin
                for (int j = SYSTOLIC_ARRAY_WIDTH-1; j >= 0; j--) begin
                    if(rd_H_time_counter >= j && rd_H_time_counter < rd_H_row_size + j && j < rd_H_col_size) begin
                        ...
                        ub_rd_H_valid_out[j] <= 1'b1;
                    end else begin
                        ub_rd_H_data_out[j] <= ub_rd_H_data_out[j];
                        ub_rd_H_valid_out[j] <= 1'b0;
                    end
                end
                ...
                // Keep H data live after the read pulse as well. This matches the
                // way bias/Y are consumed as per-column constants by later stages.
                rd_H_ptr <= 0;
                rd_H_row_size <= 0;
                rd_H_col_size <= 0;
                rd_H_time_counter <= '0;
