...
@brief  当前版本提供：
        ...
        8. 可选切换到 descriptor-compatible real TPU wrapper

        注意：
        - 当前版本已支持通过参数切换到真实 TPU core datapath wrapper。
...
module panda_soc_stage2_base_top #(
    ...
    parameter shared_sram_init_file = "no_init",
    parameter integer shared_sram_init_delay = 1,
    parameter sgn_period_mul = "true",
    parameter integer USE_TPU_STATUS_STUB = 1,
    parameter integer USE_TPU_DESC_DMA_STUB = 1,
    parameter integer USE_TPU_REAL_WRAPPER = 0,
    ...
    generate
        if(USE_TPU_REAL_WRAPPER) begin : gen_tpu_real_wrapper
        ...
        end else begin : gen_tpu_desc_fetch_dma_stub
    ...
    panda_soc_shared_mem_subsys #(
        .SHARED_SRAM_INIT_FILE(shared_sram_init_file),
        .SHARED_SRAM_INIT_DELAY(shared_sram_init_delay)
    ) panda_soc_shared_mem_subsys_u (
        .clk(clk),
        .rst(~sys_resetn),
