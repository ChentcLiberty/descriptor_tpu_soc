...
module tb_panda_soc_stage2_cpu_boot_cpu_frontend;
    ...
    localparam [31:0] EXPECT_PARAM_KEY1          = 32'h000B_FF15;
        ...
        localparam [31:0] EXPECT_CLASSIFIER_L2_OUT0  = `BREATH_CPU_FRONTEND_FIXTURE_EXPECT_CLASSIFIER_L2_OUT0;
        localparam [31:0] EXPECT_CLASSIFIER_L2_OUT31 = `BREATH_CPU_FRONTEND_FIXTURE_EXPECT_CLASSIFIER_L2_OUT31;
        localparam [31:0] EXPECT_CLASSIFIER_OUT0     = `BREATH_CPU_FRONTEND_FIXTURE_EXPECT_CLASSIFIER_OUT0;
        localparam [31:0] EXPECT_CLASSIFIER_OUT1     = `BREATH_CPU_FRONTEND_FIXTURE_EXPECT_CLASSIFIER_OUT1;
    ...
    localparam IMEM_INIT_FILE    = "/home/jjt/soc/my_soc/CPU_Copetition_tpu_soc/work/600_competition_5stage/fpga/stage2_programs/breath_tpu_soc_demo_cpu_frontend_fixture/breath_tpu_soc_demo_cpu_frontend_fixture_imem.txt";
    localparam IMEM_INIT_FILE_B0 = "/home/jjt/soc/my_soc/CPU_Copetition_tpu_soc/work/600_competition_5stage/fpga/stage2_programs/breath_tpu_soc_demo_cpu_frontend_fixture/breath_tpu_soc_demo_cpu_frontend_fixture_imem_b0.txt";
    localparam IMEM_INIT_FILE_B1 = "/home/jjt/soc/my_soc/CPU_Copetition_tpu_soc/work/600_competition_5stage/fpga/stage2_programs/breath_tpu_soc_demo_cpu_frontend_fixture/breath_tpu_soc_demo_cpu_frontend_fixture_imem_b1.txt";
    localparam IMEM_INIT_FILE_B2 = "/home/jjt/soc/my_soc/CPU_Copetition_tpu_soc/work/600_competition_5stage/fpga/stage2_programs/breath_tpu_soc_demo_cpu_frontend_fixture/breath_tpu_soc_demo_cpu_frontend_fixture_imem_b2.txt";
    localparam IMEM_INIT_FILE_B3 = "/home/jjt/soc/my_soc/CPU_Copetition_tpu_soc/work/600_competition_5stage/fpga/stage2_programs/breath_tpu_soc_demo_cpu_frontend_fixture/breath_tpu_soc_demo_cpu_frontend_fixture_imem_b3.txt";
    ...
    initial begin
        ...
        forever begin
            ...
            $display("[TB][PROG] t=%0t launch_count=%0d pc=0x%08x inst_rsp_hs=%0d data_rsp_hs=%0d dtcm_rsp_hs=%0d dbus_aw_hs=%0d dbus_ar_hs=%0d dcache_aw_hs=%0d dcache_ar_hs=%0d tpu_mode=0x%08x tpu_net=0x%08x tpu_desc=0x%08x",
                $time,
                launch_count,
                dut.panda_risc_v_min_proc_sys_u.panda_risc_v_u.panda_risc_v_ifu_u.now_pc,
                inst_rsp_hs,
    ...
    initial begin
        ...
        $display("[TB] fixture CPU-front-end shared SRAM supplied through shared_sram_init_file: %0s", PARAM_POOL_INIT_FILE);        expect_word(PARAM_KEY_INDEX + 0, EXPECT_PARAM_KEY0, "preloaded param_pool key[0] mismatch");        expect_word(PARAM_KEY_INDEX + 1, EXPECT_PARAM_KEY1, "preloaded param_pool key[1] mismatch");
        ext_resetn = 1'b1;
        ...
        expect_word(PARAM_KEY_INDEX + 0, EXPECT_PARAM_KEY0, "param_pool key[0] mismatch after CPU boot");        expect_word(PARAM_KEY_INDEX + 1, EXPECT_PARAM_KEY1, "param_pool key[1] mismatch after CPU boot");
        if(!dut.tpu_ctrl_status_done) begin
        ...
        expect_word(CLASS_L2_WORD_INDEX + 0, EXPECT_CLASSIFIER_L2_OUT0, "cpu-front-end classifier l2 output[0] mismatch");
        expect_word(CLASS_L2_WORD_INDEX + 31, EXPECT_CLASSIFIER_L2_OUT31, "cpu-front-end classifier l2 output[31] mismatch");
        expect_word(CLASS_OUT_WORD_INDEX + 0, EXPECT_CLASSIFIER_OUT0, "cpu-front-end classifier final output[0] mismatch");
        expect_word(CLASS_OUT_WORD_INDEX + 1, EXPECT_CLASSIFIER_OUT1, "cpu-front-end classifier final output[1] mismatch");

        $display("[TB] cpu boot launched all twenty-one stages with fixture CPU-front-end path");
        $display("[TB] CPU top-level stage2 fixture CPU-front-end boot test passed");
        repeat(20) @(posedge clk);
        $display("[TB] fixture CPU-front-end shared SRAM supplied through shared_sram_init_file: %0s", PARAM_POOL_INIT_FILE);        expect_word(PARAM_KEY_INDEX + 0, EXPECT_PARAM_KEY0, "preloaded param_pool key[0] mismatch");        expect_word(PARAM_KEY_INDEX + 1, EXPECT_PARAM_KEY1, "preloaded param_pool key[1] mismatch");
        $finish;
    end
