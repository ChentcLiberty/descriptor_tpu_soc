...
launch + desc_base
  ...
  -> 默认按 net_id 固定表抓 param blob；flags[16] tile 模式按 output_words*(2*input_words+1) 抓 param blob
...
- 再按 `net_id` 对应固定表抓 param blob；如果 `flags[16]` 进入 `TILE2X2_Q8_8` 模式，则按 `output_words * (2 * input_words + 1)` 动态抓参数
...
- 当前 output 已有两种模式：
  ...
  - `flags[16]` tile 模式：执行 multi-tile 2x2 Q8.8 MAC，输入/权重/偏置按两个 16-bit lane 打包；每个 output word 对应一个 2-output tile，参数消耗 3 word，`param_words = output_words * (2 * input_words + 1)`
...
- 新增 `2 -> 32` 第一层形态 directed test：`input_words=1/output_words=16`，DMA 自动拉取 `48` 个参数并写回 16 个 packed output word
- 新增 multi-input packed linear directed test：`input_words=2/output_words=2`，DMA 自动拉取 `10` 个参数，验证跨多个 input word 累加
...
4. `tb_tpu_desc_fetch_dma_stub.sv` 已通过 `VCS` 编译 + 运行
   - 实际仿真输出：
     ...
     - `q8.8 multi-input packed linear outputs matched expected packed results`
...
6. `tb_panda_soc_stage2_cpu_boot.sv` 已通过 `VCS` 编译 + 运行
   ...
   - 实际仿真输出：
     - `observed launch #1/#2/#3/#4/#5/#6/#7/#8.../#21`
     - launch #1~#5 已由 CPU 软件以 `flags=0x00010001` 连续触发 `NET_ID=0` 的 `TILE2X2_Q8_8` 模式，执行 `MLP_KEY: 2 -> 32 -> 64 -> 128 -> 64 -> 32`
     - launch #6~#7 已连续触发 `NET_ID=1`，执行 `MLP_OTHER: 6 -> 32 -> 32` 的两层 deterministic tile schedule
     - launch #8~#21 已连续触发 `NET_ID=2`，执行 `CLASSIFIER: 322 -> 256 -> 128 -> 64 -> 4` 的 deterministic 分块 tile schedule
     - `322 -> 256` 拆成 8 个 output chunk，`256 -> 128` 拆成 4 个 output chunk，后两层各 1 个 chunk
     - launch 时 `desc/input/param` 在 shared SRAM 中均为非零且内容正确，classifier 参数区为 `0x6002_0000/0x6005_0000/0x6006_4000/0x6006_9000`
     - `cpu boot launched all twenty-one stages and shared SRAM contents match expectations`
     - `CPU top-level stage2 boot test passed`
   - 2026-04-17 已在软件重编译与 IMEM 重新生成后再次通过该 CPU boot 顶层仿真，IMEM 镜像大小为 `6972` bytes
   - 新增 `work/600_competition_5stage/scripts/check_breath_tpu_soc_golden.py`，不依赖 `torch`，用于软件侧复现 21 次 launch 的 deterministic Q8.8 调度、descriptor 顺序、param_pool 分块和关键输出
   - `tpu_runtime.c` 已新增 `TPU_RUNTIME_USE_EXPORTED_PARAMS_Q8_8` 可选分支；默认值为 `0`，保持 deterministic 参数；置 `1` 时加载导出脚本生成的 `software/generated/breath_tpu_params_q8_8.h`
   - CPU boot TB 已检查完整 `MLP_KEY`、`MLP_OTHER` 和 `CLASSIFIER` deterministic schedule 的关键输出；当前还不是导入真实训练权重后的 golden accuracy 测试。

7. `tpu_mlp_compute_stub.v` 已接入 DMA 写回路径
   - 默认路径仍保留 checksum-compatible placeholder，保证未启用 `TILE2X2_Q8_8` 的 classifier 占位路径兼容
   - `flags[16]` 已启用 packed Q8.8 linear tile 模式，完成从 input/param 到 packed output 的真实乘加
   - compute stub 现在会缓存多个 packed input word，并对每个 output word 做跨 `input_words` 的累加；`flags[0]` 在 tile 模式下执行 ReLU clamp
   - tile 模式参数长度由 descriptor 的 `output_words` 推导：每个 output word 消耗 `2 * input_words + 1` 个 param word，因此 `2 -> 32` 第一层形态为 `input_words=1 / output_words=16 / param_words=48`，`64 -> 128` 最大 demo 层为 `input_words=32 / output_words=64 / param_words=4160`
...
- 真实 CPU 程序已经通过 `0x4000_4000` 发起 21 次 launch，并驱动 shared SRAM descriptor/input/param 路径
...
software deterministic golden check：
...
1. 把 real-params 预加载路径从 SoC 顶层 `shared_sram_init_file` 仿真初始化继续推进成板上可用的 bootloader/ROM/外部配置流；当前可用 `work/600_competition_5stage/scripts/prepare_breath_tpu_real_param_preload.py` 一键重生仿真产物
2. 把 DMA 原型的 input/param fetch 结果转成 TinyTPU 可消费的 UB/load 接口
3. 后续再把 tile 内部替换为真实 TinyTPU systolic/PE，并保持 `NET_ID=0/1/2` 的 21 次 launch 回归
4. `tb_panda_soc_stage2_smoke.sv` 可后补，当前优先级低于真实 compute datapath 替换

## 10. CPU boot real Q8.8 参数预加载验证（2026-04-18）

当前已经在不破坏确定性基线的前提下，新增了真实导出参数的 shared SRAM 预加载验证路径：

- 新增 `TPU_RUNTIME_PARAM_POOL_PRELOADED` 构建开关；该开关打开时，CPU runtime 不再写 `param_pool`，只做 shared SRAM 同步，避免 1MB 级参数 header 塞进 CPU 镜像导致 flash 溢出。
- 真实 Q8.8 参数仍由 `software/generated/breath_tpu_params_q8_8.h` 保存；仿真路径通过 `scripts/gen_breath_tpu_param_pool_init.py` 转成 sparse `$readmemh` 文件：`fpga/stage2_programs/breath_tpu_params_q8_8/breath_tpu_param_pool_q8_8.mem`。
- 新增 `shared_sram_init_file` 顶层参数并透传到 `panda_soc_shared_mem_subsys`；real-param CPU boot TB 使用 `breath_tpu_soc_demo_preload_params` IMEM，并通过该顶层参数加载 `param_pool`，不再由 TB 直接层次化 `$readmemh` 写 `axi_ram_u.mem`。
- 新增 run script：`run_vcs_stage2_cpu_boot_real_params.sh`。
- real-params CPU boot VCS 已通过：CPU 发起 21 次 TPU launch，最终 classifier output 为 `0x001CFE5C 0xFE3D03B3`。
- 原有确定性参数 CPU boot VCS 已重新通过：CPU 发起 21 次 TPU launch，最终 classifier output 仍为 `0x02000100 0x04000200`。

这个结果说明：

- `descriptor -> DMA -> shared SRAM -> TILE2X2_Q8.8 MAC -> output writeback` 对确定性参数和真实导出参数都闭合了。
- 真实参数当前适合走 ROM/TB/bitstream 初始化或外部 loader 预灌路线，不适合直接编译进当前 CPU 镜像。
