...
- 当前 Stage2 RTL 和 CPU 软件 demo 已验证 `flags[16]` 的 packed Q8.8 linear 模式；CPU boot 顶层已经顺序跑通关键特征分支 `2 -> 32 -> 64 -> 128 -> 64 -> 32`，其中最大层 `64 -> 128` 使用 `input_words=32 / output_words=64 / param_words=4160`，unit test 还验证了 `input_words=2` 的跨输入累加。
...
```text
0x6000_0000  param_pool.mlp_key_l0
0x6000_0400  param_pool.mlp_key_l1
0x6000_2000  param_pool.mlp_key_l2
0x6000_7000  param_pool.mlp_key_l3
0x6000_C000  param_pool.mlp_key_l4
0x6000_E000  param_pool.mlp_other_l0
0x6000_E400  param_pool.mlp_other_l1
0x6002_0000  param_pool.classifier_l0_chunk0
0x6005_0000  param_pool.classifier_l1_chunk0
0x6006_4000  param_pool.classifier_l2
0x6006_9000  param_pool.classifier_l3
0x6007_0000  param_pool.cnn1d_reserved
```



### 6.1 当前真实参数加载结论（2026-04-18）

真实 Q8.8 参数已经可以导出并用于 CPU boot 顶层仿真，但参数体量不适合直接编译进当前 CPU 镜像。当前采用两条路径并存：

- 默认确定性路径：CPU 在 `tpu_load_param_pool()` 中写入 identity-style 参数，用于稳定回归和地址/descriptor/tile 调度检查。
- 真实参数路径：构建时打开 `TPU_RUNTIME_PARAM_POOL_PRELOADED=1`，CPU 不写参数池；当前仿真通过 SoC 顶层 `shared_sram_init_file` 把 sparse 参数 mem 预灌到 shared SRAM，后续上板替换成 bootloader/ROM/外部配置流。

当前 real-params CPU boot 顶层仿真已经跑通 21 次 TPU launch，最终分类头输出为 `0x001CFE5C 0xFE3D03B3`。
...
当前 RTL 已落地一个受控的 packed linear 模式：`flags[16] = TPU_DESC_F_TILE2X2_Q8_8` 时，DMA 不再按固定表抓参数，而是按 `output_words * (2 * input_words + 1)` 抓参数；每个 output word 是两个 Q8.8 输出 lane，compute 会跨多个 input word 累加。
...
- 当前已验证到 CPU 可通过 C 程序发起 21 次 TPU launch：完整 `MLP_KEY: 2 -> 32 -> 64 -> 128 -> 64 -> 32`、`MLP_OTHER: 6 -> 32 -> 32`、`CLASSIFIER: 322 -> 256 -> 128 -> 64 -> 4` 已进入 `descriptor + DMA + shared SRAM + AXI arbiter` 闭环。
- 当前还新增了不依赖 `torch` 的 deterministic software golden，专门检查 21 次 launch 的 Q8.8 计算、descriptor 顺序、param_pool 分块和关键输出；真实 Q8.8 权重已经可通过 `export_breath_linear_q8_8_params.py --loader auto` 导出，并可用 `prepare_breath_tpu_real_param_preload.py` 生成 shared SRAM 预加载产物。预加载已从 TB 层次化 `$readmemh` 收敛到 SoC 顶层 `shared_sram_init_file` 仿真初始化入口；下一步是推进板上 loader/ROM 路径，并扩展真实模型 golden 样本。
